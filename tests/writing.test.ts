...
import {
  loadCharacters,
  requireCharacter,
} from "../src/writing/character-file.js";
import { archivePath, saveProse } from "../src/writing/archive.js";
import {
  erroringStreamModel,
  reasoningThenTextModel,
  says,
  scriptedModel,
  testConfig,
...
function databasePath() {
  return join(mkdtempSync(join(tmpdir(), "sonde-write-")), "sonde.db");
}
...
const NOIR = {
  noir: {
    name: "Noir",
    summary:
      "First person, past tense, a narrator who notices what he would rather not.",
...
describe("writing workflow seams", () => {
  test("upgrades a legacy run database once and labels historical rows research", () => {
    ...
    legacy.exec(
      "CREATE TABLE runs (id TEXT PRIMARY KEY, question TEXT NOT NULL, started_at INTEGER NOT NULL, finished_at INTEGER, stopped_by TEXT, usd REAL, total_tokens INTEGER, search_credits INTEGER, report_json TEXT)",
    );
    legacy
      .prepare("INSERT INTO runs (id, question, started_at) VALUES (?, ?, ?)")
      .run("old", "old question", 1);
    legacy.close();
    const db = openDb(path);
    expect(new RunStore(db).recent()).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: "old", kind: "research" }),
      ]),
    );
    db.close();
  ...
  test("writes a complete whole piece and records it as writing", async () => {
    ...
    try {
      expect(new RunStore(db).recent()[0]).toMatchObject({
        kind: "writing",
        question: "Write a note",
      });
      expect(
        db
          .prepare("SELECT report_json FROM runs WHERE id = ?")
          .get(result.runId),
  ...
  test("marks a reasoning-only response incomplete instead of claiming an empty write completed", async () => {
    const dbPath = databasePath();
    const model = reasoningThenTextModel(
      ["Planning the piece without ever producing it."],
      [],
    );
    const events: WriteEvent[] = [];
    const result = await runWrite({
  ...
  test("reserves output space for both configured reasoning and requested prose", async () => {
    const model = scriptedModel([says("Finished prose.")]);
    await runWrite({
      brief: "Write a note",
      length: 800,
  ...
  test("delivers prose cut off by the output-token ceiling as incomplete", async () => {
    ...
    expect(result.warnings.join(" ")).toContain(
      "output token limit (finish reason: length; provider: max_tokens)",
    );
  });
  ...
  test("delivers streamed partial prose after an in-stream provider error", async () => {
    ...
    const result = await runWrite({
      brief: "Write",
      config: testConfig(dbPath),
      model: erroringStreamModel("upstream stopped", "Useful beginning."),
      onEvent: (event) => {
        events.push(event);
      },
    });
  ...
  test("names a saved piece by local time, mode, and run id", () => {
    ...
    expect(archivePath(".sonde/writing", "new", "run_ab12cd34", at)).toBe(
      ".sonde/writing/20260906-210233-new-run_ab12cd34.md",
    );
    ...
    expect(archivePath("w", "expand", "run_x", at)).toBe(
      "w/20260906-210233-expand-run_x.md",
    );
  });

  test("saves the prose alone, so continue can read the file back as a draft", () => {
    const path = join(
      mkdtempSync(join(tmpdir(), "sonde-archive-")),
      "deep",
      "piece.md",
    );
  ...
  test("takes its register from the draft when continue and expand are given no style", async () => {
    ...
    expect(model.prompts[0]).toContain(
      "Take the register from the draft itself",
    );
  ...
  test("lets a styles file entry replace the built-in style it is named for", async () => {
    ...
    await runWrite({
      ...
      config: configWithStyles({
        plain: {
          summary: "Numbers, not adjectives.",
          moves: ["Say the number."],
        },
      }),
    });
    const prompt = model.prompts[0]!;
  ...
  test("takes the id as the name when the styles file gives none", () => {
    const config = configWithStyles({
      terse: { summary: "Short.", moves: ["Stop early."] },
    });
    expect(loadStyles(config.stylesPath).terse).toMatchObject({
      id: "terse",
      name: "terse",
      avoid: [],
    });
  ...
  test("leaves the built-in styles alone when there is no styles file", () => {
    expect(
      Object.keys(loadStyles(testConfig(databasePath()).stylesPath)).sort(),
  ...
  test("names the file and the field when a styles file is wrong", () => {
    ...
    const stray = configWithStyles({
      noir: { summary: "Dark.", moves: ["Stop."], tone: "grim" },
    });
  ...
  test("answers an unknown style with the styles this machine actually has", () => {
    ...
    expect(() => requireStyle(catalogue, "nior")).toThrow(
      /Unknown style: nior/u,
    );
  ...
  test("reads length as words in every mode", () => {
    ...
    expect(
      writePrompt({ brief: "b", mode: "new", style, length: 500 }),
    ...
    expect(
      writePrompt({ brief: "b", draft: "d", mode: "expand", style }),
    ).toContain("Choose an appropriate length");
  });
  ...
  test("uses an independent writing model and reasoning default", () => {
    const config = loadConfig({
      ...
      SONDE_WRITER_MODEL: "writer",
      SONDE_WRITE_MODEL: "writer-prose",
    });
    expect(config.writeModel).toBe("writer-prose");
    expect(config.writeReasoningEffort).toBe("medium");
  });

  describe("character reference", () => {
    test("leaves the prompt exactly as it was when no characters are given", () => {
      const withoutField = writePrompt({
        brief: "b",
        mode: "new",
        style: WRITE_STYLES.plain,
      });
      const withEmptyList = writePrompt({
        brief: "b",
        mode: "new",
        style: WRITE_STYLES.plain,
        characters: [],
    ...
    test("adds a bounded character block between the style and the house rules", async () => {
      ...
      expect(prompt.indexOf("Style — Plain")).toBeLessThan(
    ...
    test("rejects a characters file with an unknown field, naming the file and the field", () => {
      const bad = configWithCharacters({
        mara: { name: "Mara", description: "d", secretAgenda: "kill everyone" },
      });
      expect(() => loadCharacters(bad.charactersPath)).toThrow(
        bad.charactersPath,
      );
    ...
    test("answers an unknown character id with the cast this machine actually has", () => {
      const catalogue = loadCharacters(
        configWithCharacters({ mara: { name: "Mara", description: "d" } })
          .charactersPath,
      );
      expect(() => requireCharacter(catalogue, "sal")).toThrow(
        /Unknown character: sal/u,
      );
    ...
    test("refuses a field over the per-field character limit before the model is called", async () => {
      const config = configWithCharacters({
        mara: { name: "Mara", description: "d".repeat(50) },
      });
    ...
    test("refuses a characters file over the byte limit before the model is called", async () => {
      const config = configWithCharacters({
        mara: { name: "Mara", description: "d".repeat(1000) },
      });
    ...
    test("records the injected characters and a file hash on write_start, readable back from run_events", async () => {
      const config = configWithCharacters({
        mara: { name: "Mara Okonkwo", description: "d" },
      });
      const events: WriteEvent[] = [];
      const result = await runWrite({
        brief: "Write a scene",
        characters: ["mara"],
        config,
        model: scriptedModel([says("Scene.")]),
        onEvent: (event) => {
          events.push(event);
        },
      });
      ...
      const charactersHash =
        start && "charactersHash" in start ? start.charactersHash : undefined;
      expect(charactersHash).toMatch(/^[0-9a-f]{16}$/u);
      const db = openDb(config.dbPath);
      try {
        const row = db
          .prepare(
            "SELECT payload FROM run_events WHERE run_id = ? AND type = 'write_start'",
          )
