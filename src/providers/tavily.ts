...
export class TavilyProvider implements SearchProvider, ContentFetcher {
  ...
  async search(query: string, options: SearchOptions = {}): Promise<SearchOutcome> {
    ...
    return {
      ...
      creditsUsed: res.usage?.credits ?? estimateSearchCredits(this.searchDepth),
    };
  }

  async fetch(urls: string[], options: { query?: string } = {}): Promise<FetchOutcome> {
    ...
    return {
      pages,
      failures,
      creditsUsed: res.usage?.credits ?? estimateExtractCredits(urls.length, this.extractDepth),
    };
  }
}
...
function estimateExtractCredits(urlCount: number, depth: string): number {
  ...
  return Math.max(1, Math.ceil(urlCount / perCredit)) * (depth === 'advanced' ? 2 : 1);
}
